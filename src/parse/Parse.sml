...
(* type grammar *)
val the_type_grammar = ref @{position} type_grammar.min_grammar
val type_grammar_changed = ref @{position} false
...
val the_term_grammar = ref @{position} term_grammar.min_grammar
val term_grammar_changed = ref @{position} false
...
val current_backend : PPBackEnd.t ref =
    ref @{position} (if !Globals.interactive then interactive_ppbackend()
...
val the_lty_grm = ref @{position} type_grammar.empty_grammar
val the_ltm_grm = ref @{position} term_grammar.stdhol
...
in
val type_parser1 =
    ref @{position} (parse_type termantiq_constructors false (type_grammar()))
val type_parser2 =
    ref @{position} (parse_type typantiq_constructors false (type_grammar()))
end
...
val type_printer = ref @{position} (type_pp.pp_type (type_grammar()))

val grammar_term_printer =
  ref @{position} (term_pp.pp_term (term_grammar()) (type_grammar()))
fun pp_grammar_term pps t = (!grammar_term_printer) (!current_backend) pps t

val term_printer = ref @{position} pp_grammar_term
...
val grammarDB_value =
    ref @{position} (Binarymap.mkDict String.compare :(string,grammarDB_info)Binarymap.dict)
...
val the_absyn_parser: (term frag list -> Absyn.absyn) ref =
    ref @{position} (TermParse.absyn (!the_term_grammar) (!the_type_grammar))
...
val grm_updates = ref @{position} [] : (string * string * term option) list ref;
...
val unicode_off_but_unicode_act_complaint = ref @{position} true
