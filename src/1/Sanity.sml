...
val report_last_thm = ref @{position} "";
...
val report_verbose = ref @{position} true;
...
val accepted_axioms = ref @{position} ([]:string list);
val accepted_oracles = ref @{position} (["DISK_THM"]:string list);
...
val check_thm_name_flag = ref @{position} true;
...
val check_var_names___const_flag = ref @{position} true;
...
val check_free_vars_ref = ref @{position} 1;
...
val strict_sanity = ref @{position} true;
