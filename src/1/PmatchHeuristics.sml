...
let
  val heuristicL_ref = ref @{position} ([]:pmatch_heuristic list)
  fun add_heu heu = (heuristicL_ref := heu :: (!heuristicL_ref))

  fun heu (prefix : (bool * int * int) list) : pmatch_heuristic =
  let
    val current_prefix = ref @{position} prefix
    val remaining_prefix = ref @{position} prefix
...
let
  ...
  val res = ref @{position} res_input
  fun man_choice rowL =
    let
      val r = case (!res) of
          [] => 0
...
(*----------------------------------------------------------------------------
      A ref @{position}erence to store the current heuristic_fun
 ----------------------------------------------------------------------------*)

val pmatch_heuristic = ref @{position} default_heuristic_fun
val classic = ref @{position} false
