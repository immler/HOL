...
fun profile_exn_opt do_exn do_ok do_both nm f x =
  let
     ...
     val _ = case result of
                OK _ => if do_ok then add_profile (nm ^ "_OK") timefx else ()
              | Ex e =>
                  (case do_exn of
                      NONE => ()
...
fun output_profile_results outstr results =
   let
      fun foldl_map_this
             ((nm_width, real_width, usr_width, sys_width, gc_width, n_width),
             (nm, {usr, sys, gc, real, n})) =
       let
          val usr = Time.toString usr
      ...
      val ((nm_width, real_width, usr_width, sys_width, gc_width, n_width),
           strings) =
      ...
      fun app_this (nm, real, usr, sys, gc, n) =
         (print (StringCvt.padRight #" " nm_width nm)
          ; print " "
          ; print (StringCvt.padLeft #" " n_width n)
          ; print " "
          ; print (StringCvt.padLeft #" " real_width real)
          ; print " "
          ; print (StringCvt.padLeft #" " usr_width usr)
          ; print " "
          ; print (StringCvt.padLeft #" " sys_width sys)
          ; print " "
          ; print (StringCvt.padLeft #" " gc_width gc)
          ; print "\n")
   in
      List.app app_this
         (("Label", "real", "user", "system", "gc", "#calls") :: strings)
   end

fun output_profile_result outstr result = output_profile_results outstr [result]
